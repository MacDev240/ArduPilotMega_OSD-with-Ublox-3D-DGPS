...
{
	Serial.printf_P(PSTR("CMDs:\n"
						 "  logs\n"
						 "  setup\n"
						 "  test\n"
 						 "  planner\n"
 						 "\n"
						 "Move CLI/FLY sw & reset to FLY\n"
						 "\n"));
	return(0);
}
...
MENU(main_menu, "ArduCopter 2.0.45 Heli OSD beta", main_menu_commands);
...
static void init_ardupilot()
{
	...
	if (!g.format_version.load() ||
		...
		*/
		...
		Serial.printf_P(PSTR("FW rev: erasing EEPROM\n"));
		...
		Serial.printf_P(PSTR("Run Setup...\n"));
		while (true) {
			delay(1000);
			if(motor_light){
	...
	if (check_startup_for_CLI()) {
		...
		Serial.printf_P(PSTR("\n"
							 "Inter-Active Setup\n"
							 "\n"
							 " 'help'= CMDs, 'exit'=leave\n"
							 " Run 'setup' for 1st config\n\n"));
