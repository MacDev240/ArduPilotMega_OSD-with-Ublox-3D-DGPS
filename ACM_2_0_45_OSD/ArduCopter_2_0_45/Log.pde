...
// Write an GPS packet. Total length : 32 bytes  mjc
static void Log_Write_GPS()
{
	...
	DataFlash.WriteByte(g_gps->h_dop); // with UBLOX_DL mjc			// 4a
	DataFlash.WriteByte(g_gps->hdop);					// 4

	DataFlash.WriteLong(current_loc.lat);					// 5
	DataFlash.WriteLong(current_loc.lng);					// 6
	DataFlash.WriteLong(current_loc.alt);					// 7
	DataFlash.WriteLong(g_gps->altitude);					// 8

	DataFlash.WriteInt(g_gps->ground_speed);				// 9
	DataFlash.WriteInt((uint16_t)g_gps->ground_course);			// 10
...
{
	Serial.printf_P(PSTR("GPS, %ld, %d, %d, %d, %d,"
					  	"%4.7f, %4.7f, %4.4f, %4.4f, "
					  	"%d, %u\n"),
						...
						(int)DataFlash.ReadByte(),		// 4a h_dop  with UBLOX_DL mjc
						(int)DataFlash.ReadByte(),		// 4  hdop

						(float)DataFlash.ReadLong() / t7,	// 5 lat
						(float)DataFlash.ReadLong() / t7,	// 6 lon
						(float)DataFlash.ReadLong() / 100.0,	// 7 gps alt
						(float)DataFlash.ReadLong() / 100.0,	// 8 sensor alt

						DataFlash.ReadInt(),			// 9 ground speed
						(uint16_t)DataFlash.ReadInt());		// 10 ground course
}
