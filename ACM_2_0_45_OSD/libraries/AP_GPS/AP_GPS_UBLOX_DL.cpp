...
#include "AP_GPS_UBLOX_DL.h" // Adds Differential 3D  Lock and uBlox DOP message mjc
#include <stdint.h>

// Constructors ////////////////////////////////////////////////////////////////

AP_GPS_UBLOX_DL::AP_GPS_UBLOX_DL(Stream *s) : GPS(s)
{
}
		...
		case 3:
			...
			if (_gather) {						// if class was interesting
				...
				case MSG_STATUS:
					...
					break;
					case MSG_DOP:				//new GPS_LOCK message for UBLOX_DL mjc
					_expect = sizeof(ubx_nav_dop);
					break;						
				case MSG_SOL:
...
bool
AP_GPS_UBLOX_DL::_parse_gps(void)
{
	switch (_msg_id) {
	case MSG_POSLLH:
	...
	case MSG_STATUS:
		...
		fix_type	= _buffer.status.fix_type;//mjc for HK_GCS
		dgps_fix	= _buffer.status.differential_status; //mjc for HK_GCS
		break;
	case MSG_DOP:
	...
	case MSG_SOL:
		...
		//fix_type	= _buffer.solution.fix_type;//mjc for HK_GCS
		num_sats	= _buffer.solution.satellites;
		hdop		= _buffer.solution.position_DOP; //mjc actually p_dop on std ublox
		break;
	case MSG_VELNED:
