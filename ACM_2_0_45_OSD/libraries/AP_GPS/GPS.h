...
public:
	...
	enum GPS_Status {
		...
		GPS_OK = 2,		///< Receiving valid messages and 2D or 3D lock mjc
		GPS_DL = 3		///< Receiving valid messages and 3D/DGPS lock mjc
	};
	...
	int		hdop;			///< horizontal dilution of precision in cm // Note:  hdop = positional_DOP for UBLOX_DL mjc
	int		h_dop;			///< horizontal dilution of precision in cm
	int		p_dop;			///< position dilution of precision in cm
	int		v_dop;			///< vertical dilution of precision in cm
	uint8_t	fix_type;		///< 0=None 1=Dead Reckoning 2=2D 3=3D 4=3D/DGPS 5=Time Only
	uint8_t	dgps_fix;		///< 3D/DGPS fix status 0=No 1=Yes		
	...
	virtual void setHIL(long time, float latitude, float longitude, float altitude,
						float ground_speed, float ground_course, float speed_3d, uint8_t num_sats); //, uint8_t h_dop);
