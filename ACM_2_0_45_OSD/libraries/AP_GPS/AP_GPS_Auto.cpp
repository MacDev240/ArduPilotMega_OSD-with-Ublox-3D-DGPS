...
AP_GPS_Auto::_detect(void)
{
	...
	for (tries = 0; tries < 2; tries++) {
		...
		fingerprint[4] = _port->read(); // uBlox sending DOP message 0x04 & 3D Differential Lock status mjc
		...
		if ((0xb5 == fingerprint[0]) &&
			...
			// any other message is ublox, check for DOP message
			if (0x04 == fingerprint[4]) { // detect uBlox DOP message sent
				gps = new AP_GPS_UBLOX_DL(_port);
				Serial.print_P(PSTR(" ublox_dl ")); // 
				break;
			}
