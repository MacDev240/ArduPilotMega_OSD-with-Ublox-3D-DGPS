...
//
#ifndef AP_GPS_UBLOX_DL_h
#define AP_GPS_UBLOX_DL_h

#include <GPS.h>

#define UBLOX_SET_BINARY	"$PUBX,41,1,0003,0001,38400,0*26"

class AP_GPS_UBLOX_DL : public GPS
{
public:
    // Methods
	AP_GPS_UBLOX_DL(Stream *s); // with DOP message 0x04 mjc
	virtual void	init();
	virtual bool	read();
...
#pragma pack(1)
	...
	struct ubx_nav_solution {
		...
		uint16_t	position_DOP;		// p_dop sent as hdop value to GCS when using standard UBLOX mjc
		uint8_t		res;
		uint8_t		satellites;
		uint32_t	res2;
	};
...
#pragma pack(pop)
	enum ubs_protocol_bytes {
		...
		MSG_POSLLH = 0x2,
		MSG_STATUS = 0x3,
		MSG_DOP = 0x4,		// supports new GPS_LOCK message mjc
		MSG_SOL = 0x6,
		MSG_VELNED = 0x12
	};
	...
	union {
		...
		ubx_nav_dop			dop; // supports new GPS_LOCK message mjc
